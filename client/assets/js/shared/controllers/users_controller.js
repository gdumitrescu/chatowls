...
app.controller("UsersController", function($scope, $firebase) {
	...
	ref.orderByChild("expires").on("child_added", function(snapshot) {
	  if(snapshot.val().provider == "facebook" && snapshot.val().facebook.cachedUserProfile.locale != undefined){
	  	...
	  	var user = { 'name': snapshot.val().facebook.cachedUserProfile.first_name , 'lang' : snapshot.val().facebook.cachedUserProfile.locale, 'provider' : 'fb'};
	  	...
	  	var user = { 'name': snapshot.val().twitter.cachedUserProfile.login , 'lang' : snapshot.val().github.cachedUserProfile.location, 'provider':'gh'};
	  	$scope.$apply(function(){
	  		$scope.users.push(user);
	  	});
	  }
	  	...
	  	var user = {'name': snapshot.val().twitter.cachedUserProfile.name , 'lang' : snapshot.val().twitter.cachedUserProfile.lang, 'provider':'tw'};
	  	$scope.$apply(function(){
	  		$scope.users.push(user);
	   });
	  }
	  	...
	  	var user = {'name':snapshot.val().google.cachedUserProfile.given_name, 'lang':snapshot.val().google.cachedUserProfile.locale, 'provider':'gl' };
	  	$scope.$apply(function(){
	  		$scope.users.push(user);
	    });
	  }
